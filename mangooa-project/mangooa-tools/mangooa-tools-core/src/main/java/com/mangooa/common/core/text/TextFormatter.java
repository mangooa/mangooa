...
public class TextFormatter {
	 ...
	 * @return 格式化后的消息。
	 */
	protected static String format(final String msg, final Object... values) {
		if (StringUtil.isBlank(msg) || ArrayUtil.isEmpty(values)) {
			return msg;
		}
		final int length = msg.length() + values.length;
		// 设置缓存大小提高性能
		...
		for (int idx = 0; idx < values.length; idx++) {
			...
			if (index > 0 && msg.charAt(index - 1) == StringUtil.BACKSLASH) {
				if (index > 1 && msg.charAt(index - 2) == StringUtil.BACKSLASH) {
					...
					builder.append(StringUtil.toString(values[idx]));
					position = index + 2;
				} else {
					// 占位符被转义
					idx--;
