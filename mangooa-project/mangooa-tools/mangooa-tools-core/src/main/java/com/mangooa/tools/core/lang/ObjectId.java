...
public final class ObjectId implements Comparable<ObjectId>, Serializable {
	...
	private static final long serialVersionUID = 7778351820300859356L;

	private static final int HEX_FF000000 = 0xff000000;

	private static final int DEC_24 = 24;
	...
	private ObjectId(
		...
		if ((machineIdentifier & HEX_FF000000) != 0) {
			throw new IllegalArgumentException("The machine identifier must be between 0 and 16777215 (it must fit in three bytes).");
		}
		if (checkCounter && ((counter & HEX_FF000000) != 0)) {
	...
	public ObjectId(final byte[] bytes) {
		this(ByteBuffer.wrap(Assert.notNull(bytes, "bytes must be not null")));
	}
	 ...
	 */
	ObjectId(final int time, final int machineAndProcessIdentifier, final int counter) {
		this(legacyToBytes(time, machineAndProcessIdentifier, counter));
	}
	...
	public ObjectId(final ByteBuffer buffer) {
		Assert.notNull(buffer, "buffer must be not null");
		Assert.isTrue(buffer.remaining() >= 12, "buffer.remaining() >=12");
		// Note: Cannot use ByteBuffer.getInt because it depends on tbe buffer's byte order and ObjectId's are always in big-endian order.
